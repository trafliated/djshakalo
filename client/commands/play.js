...
module.exports = {
    run: async (args, message) => {
        if (!args[0]) return message.channel.send(new discord.MessageEmbed().setColor("PURPLE").setTitle("Usage: `play <url/song title>`"));
        if (!message.member.voice.channel.id) return message.channel.send(`You must be in a voice channel!`);

        if (!queues[message.guild.id]) queues[message.guild.id] = new Queue(message.guild.id, message.member.voice.channel.id, message.channel);
